...
**Chord symbols are jazz shorthand: `^` is major, `-` or `m` is minor, `o` is
diminished.** An unrecognised symbol does not throw, it voices to **silence** -
so `Cmaj7`, `Csus4` and `Cdim` produce nothing at all while the code still looks
correct. Write `C^7`, `Csus`, `Co`. The full verified table is in `/theory`, and
`node scripts/check.mjs <file>` names any chord that came out silent.

```javascript
// Define a progression (angle brackets = one chord per cycle)
let prog = "<Cm7 Fm7 Ab^7 G7>"
...
**`.duckorbit(n)` ducks orbit `n`. It does not mean "duck me when orbit n
plays".** Read it as an active verb: the layer carrying it is the one doing the
ducking, and `n` names its victim. Getting this backwards is easy and almost
silent, so the rule is simple:

> Put `.duckorbit(N)` on the **kick**, and `.orbit(N)` on the layer that should
> pump. The ducker points at the ducked.

```javascript
.duckorbit(2)      // whenever THIS plays, duck everything on orbit 2
.duckdepth(0.4)    // how far down (0-1)
.duckattack(0.1)   // how fast it recovers
```

Two traps worth knowing:

- **Orbit 1 is the default bus.** Any layer that never calls `.orbit()` lives
  there. So `.duckorbit(1)` on a pad ducks the pad itself, plus the kick, plus
  everything else that stayed on the default. Always give the pumped layer its
  own explicit orbit.
- **Ducking an orbit nothing plays on does nothing**, and Strudel only logs
  `duck target orbit does not exist` rather than failing.

```javascript
// Classic house sidechain: chords pump under the kick
const kick = s("bd*4").bank("RolandTR909").gain(.8)
  .duckorbit(2).duckdepth(.4)        // the kick ducks orbit 2

const chords = chord("<Cm7 Fm7>").voicing().s("gm_epiano1")
  .orbit(2)                          // ...which is where the chords live
  .gain(.5)

stack(kick, chords)
```

`node scripts/check.mjs <file>` fails the track if a layer ducks its own bus.
