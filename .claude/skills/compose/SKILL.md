...
Two checks, in this order. `node scripts/check.mjs <file>` first - it evaluates
the code and names anything that voiced to silence or routed a sidechain the
wrong way, which no amount of listening will reveal. Then sound-check with
`node scripts/listen.mjs 10` once the core groove exists and fix what the NOTES
flag before building the full arrangement on top. `push.mjs` runs the first one
for you and refuses to push on errors.
