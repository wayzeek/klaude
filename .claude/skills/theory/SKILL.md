...
**`chord()` speaks jazz shorthand, not pop shorthand.** A caret means major, a
hyphen means minor, `o` is diminished and `h` is half-diminished. This matters
more than it sounds: a symbol outside its vocabulary does not error, it voices
to **silence**, so the chord just is not there while the code still reads as if
the harmony were written. `Cmaj7`, `Csus4` and `Cdim` are all silent.

| Symbol | Notes | Character |
|--------|-------|-----------|
| `C` | major triad | plain bright |
| `Cm` | minor triad | plain sad |
| `C5` | root + fifth | power chord, no mood of its own |
| `C7` | dominant 7th | bluesy, wants to move |
| `Cm7` | minor 7th | soulful, smooth - house/deep staple |
| `C^7` (or `CM7`) | major 7th | dreamy, warm - lo-fi staple |
| `Cm9` / `C^9` | add the 9th | lusher versions of the above |
| `Csus` | suspended 4th | open, unresolved, neither happy nor sad |
| `C2` | suspended 2nd | airy, unresolved, brighter than `Csus` |
| `C7sus` / `C9sus` | dominant, no 3rd | suspended tension that wants to land |
| `Co` / `Co7` | diminished / dim 7th | anxiety, horror, passing tension |
| `Ch7` | half-diminished | wistful unease, the `ii` of a minor key |
| `Caug` | augmented | uncanny, dreamlike unease |
| `C6` / `Cm6` | added 6th | vintage sweetness / noir elegance |
| `Cadd9` | triad + 9th, no 7th | open and modern, less jazzy than `C^9` |
| `C13` / `C^13` | extended | rich, big-band |

**Rule of thumb:** triads sound plain and honest; 7ths sound sophisticated; 9ths sound lush. Deep house is built on m7/m9. Lo-fi is built on `^7`. Trance/EDM drops often use plain triads for punch.

Run `node scripts/check.mjs <file>` before pushing and it names any chord that
voiced to silence, with a working spelling. Never trust a progression you have
not checked.
...
`<Dm7 G7 C^7>` - sophisticated arrival. Chain it: `<Em7 A7 Dm7 G7 C^7 C^7>`.
