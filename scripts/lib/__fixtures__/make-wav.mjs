...
/**
 * A sustained tone whose amplitude dips by `depth` at each of `kickTimes`
 * (seconds, ascending) and recovers linearly back to full level over
 * `releaseSeconds` - the shape a fast-attack sidechain compressor produces,
 * built independently of `dynamics.mjs`'s own detector so a test using this
 * fixture is not just checking the detector against its own algorithm. The
 * dip itself is instantaneous (drops to `1 - depth` in the very next sample),
 * matching a near-zero compressor attack - `dynamics.mjs`'s own calibration
 * comment (see its `PHASE_LOCK_WINDOW_SECONDS`) is built on exactly this
 * "fast dip, slower release" shape.
 */
export function duckedToneClip({ sampleRate = 44100, seconds, toneHz, kickTimes, depth, releaseSeconds, channels = 2 }) {
  const numFrames = Math.round(seconds * sampleRate)
  const mono = new Float32Array(numFrames)
  for (let i = 0; i < numFrames; i++) {
    mono[i] = 0.5 * Math.sin((2 * Math.PI * toneHz * i) / sampleRate)
  }
  const releaseFrames = Math.max(1, Math.round(releaseSeconds * sampleRate))
  for (const kickSeconds of kickTimes) {
    const start = Math.round(kickSeconds * sampleRate)
    for (let i = 0; i < releaseFrames && start + i < numFrames && start + i >= 0; i++) {
      const gain = 1 - depth + depth * (i / releaseFrames)
      mono[start + i] *= gain
    }
  }
  const samples = []
...
/**
 * A tone whose instantaneous frequency ramps linearly from `startHz` to
 * `endHz` over the clip - a textbook linear chirp, generated by integrating
 * the instantaneous frequency into phase sample-by-sample rather than the
 * naive (and wrong once frequency depends on time) `sin(2*pi*f(t)*t)`, so the
 * sweep is click-free and its rate is actually linear in time. `startHz ===
 * endHz` degenerates to a plain steady tone - the "no sweep" negative control
 * for the same detector this exists to feed.
 */
export function sweepToneClip({ sampleRate = 44100, seconds, startHz, endHz, channels = 2 }) {
  const numFrames = Math.round(seconds * sampleRate)
  const mono = new Float32Array(numFrames)
  let phase = 0
  for (let i = 0; i < numFrames; i++) {
