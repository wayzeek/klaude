 ...
 * --keep is passed. Base URL via KLAUDE_URL (default http://localhost:3000).
 */
...
const seconds = Math.min(120, Math.max(3, Number(args.find((a) => !a.startsWith('--'))) || 12))
...
async function main() {
  ...
  try {
    ...
    // Fetch the clip over HTTP (works even when KLAUDE_URL points at a
    ...
    let report
    try {
      report = formatReport(analyzeWavBuffer(clip), `live clip (${seconds}s)`)
    } catch (err) {
